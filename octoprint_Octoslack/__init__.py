...
class OctoslackPlugin(
    ...
    def get_settings_defaults(self):
        return {
            ...
            "slack_bot_config": {"token": ""},
            "slack_identity": {
                "existing_user": True, "icon_url": "", "icon_emoji": "", "username": ""
            },
            "channel": "",
    ...
    def on_print_progress(self, location, path, progress):
        try:
            progress_interval = int(
                self._settings.get(["supported_events"], merged=True).get(
                    "Progress"
                ).get(
                    "IntervalPct"
                )
        ...
        if reportPrinterState:
            ...
            if not printer_temps == None and "bed" in printer_temps:
                ...
                for key in printer_temps:
                    if key == "bed":
                        temp_str += ", Bed: " + str(
                            printer_temps["bed"]["actual"]
                        ) + unichr(
                            176
                        ) + "C/" + str(
                            printer_temps["bed"]["target"]
                        ) + unichr(
                            176
                        ) + "C"
    ...
    def process_rtm_message(self, slackAPIToken, message):
        ...
        if not processing_reaction == None:
            ...
            if (
                processing_reaction.startswith(":")
                and processing_reaction.endswith(":")
            ):
    ...
    def humanize_duration(self, total_seconds):
        ...
        if (
            len(time_str) == 0
            or (total_days == 0 and total_hours == 0 and total_minutes < 10)
        ):
            if len(time_str) > 0:
                time_str += " "
    ...
    def send_slack_message(
        ...
        if connection_method == "APITOKEN":
            ...
            slackBotToken = self._settings.get(["slack_bot_config"], merged=True).get(
                "token"
            )
        ...
        if (
            (slackAPIToken == None or len(slackAPIToken) == 0)
            and (slackWebHookUrl == None or len(slackWebHookUrl) == 0)
            and (slackBotToken == None or len(slackBotToken) == 0)
        ):
        ...
        for channel in channels.split(","):
            ...
            if not slackAPIToken == None and len(slackAPIToken) > 0:
                ...
                self._logger.debug(
                    "Slack WebHook/Bot postMessage json: " + json.dumps(slack_msg)
                )
                    ...
                    try:
                        ...
                        if event == "Progress":
                            if (
                                ...
                                self.bot_update_message(
                                    self._bot_progress_req, slack_msg
                                )
                            else:
                                self._bot_progress_req = self.bot_post_message(
                                    slack_msg
                                )
                        else:
                            self._bot_progress_req = None
